...
{#if verificationReview}
  <div class="modal-backdrop">
    <div class="gate-review" role="dialog" aria-modal="true" aria-labelledby="verification-return-title">
      ...
      <p>Compare the implementation claim with current required verification. An omission retries the same accepted contract; a conflict in governing truth enters reconciliation.</p>
      ...
      <label for="verification-route-reason"><strong>Conflict or retry note</strong></label>
      <textarea id="verification-route-reason" bind:value={verificationReason} rows="3" placeholder="Describe the governing-context conflict when reconciliation is required."></textarea>
      <div class="gate-review-actions"><Button on:click={() => verificationReview=undefined}>Not yet</Button><Button disabled={recordingReturn||!pendingReturnVerifications.length} on:click={() => evaluateReturn("retry-implementation")}>Retry implementation</Button><Button disabled={recordingReturn||!verificationReason.trim()} on:click={() => evaluateReturn("reconcile")}>Reconcile context</Button><Button variant="primary" disabled={recordingReturn||pendingReturnVerifications.length>0} on:click={() => evaluateReturn("satisfactory")}>Verification satisfactory</Button></div>
    </div>
  </div>
{/if}
