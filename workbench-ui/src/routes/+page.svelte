<script lang="ts">
  ...
  type WorkflowAction={id:string;kind:string;label:string;eligible:boolean;requiredInputs:string[];expectedOutputs:string[];target:{entityId?:string;artifactPath?:string};subject?:{entityId:string;entityType?:string};gate?:"specification"|"build-contract"|"verification";blockers:Blocker[]};
  ...
  type CompletionClaim={taskId:string;acceptedBuildContract:{id:string;fingerprint:string};implementationAttemptId:string;summary?:string;knownGaps:string[];changedFiles:string[];claims:Record<string,unknown>;checks:Array<Record<string,unknown>>;externalVerifications:Array<Record<string,unknown>>};
  type WorkflowAssessment = { featureId:string; graphAvailable:boolean; buildValid:boolean; repositoryValid:boolean; blockingQuestions:string[]; tasks:Array<{id:string;lifecycle:string;contract?:string;contractArtifactPath?:string;contractStale?:boolean;changedInputs:Blocker[]}>;verifications:Array<{id:string;satisfied:boolean;status:string;artifactPath:string}>;gates:Record<string,Gate>;actions:WorkflowAction[];governingChanges:Array<{contractId:string;inputs:Array<{id:string;reason:string}>}>;implementation:{attempts:Array<{id:string;taskId:string;attempt:number;returned:boolean;claim?:CompletionClaim}>;pendingReturns:Array<{taskId:string;implementationAttemptId:string;claim:CompletionClaim;verificationIds?:string[]}>;retryContexts:Array<{taskId:string;failedVerifications:string[];affectedAcceptanceCriteria:string[];affectedRequirements:string[];blockingFindings:string[];knownGaps:string[];contractId?:string;contractCurrent:boolean;nextEligibleAction:string}>};reconciliation:{required:boolean;reasons:Blocker[]};currentGate?:"specification"|"build-contract"|"verification";pendingGate?:"specification"|"build-contract"|"verification";primaryActionId?:string; specificationEligible:boolean; buildContractEligible:boolean; completionEligible:boolean; fingerprint:string };
  ...
  let returnClaim = "";
  let returnKnownGaps = "";
  let returnChangedFiles = "";
  let returnAssessments = '{"requirements":[],"acceptanceCriteria":[],"lockedDecisions":[]}';
  let returnChecks = "[]";
  let returnExternalVerifications = "[]";
  let verificationReview: {action:WorkflowAction;taskId:string} | undefined;
  ...
  $: currentGate = workflow?.currentGate ? workflow.gates[workflow.currentGate] : undefined;
  $: pendingGate = workflow?.pendingGate ? workflow.gates[workflow.pendingGate] : undefined;
  ...
  $: primaryAction = !workflowRunHistorical ? workflow?.actions.find((action) => action.id === workflow?.primaryActionId) : undefined;
  ...
  $: pendingImplementationReturn = workflow?.implementation.pendingReturns.find(value=>value.taskId===verificationReview?.taskId);
  $: pendingReturnVerifications = unsatisfiedVerifications.filter(value=>pendingImplementationReturn?.verificationIds?.includes(value.id));
  ...
  function authoringInstruction(action:WorkflowAction):string{const target=action.target.artifactPath??action.target.entityId??"the responsible artifact";return `Update ${target}. Use: ${action.requiredInputs.join("; ")}. Produce: ${action.expectedOutputs.join("; ")}.`;}
  function gateForAction(action:WorkflowAction):Gate|undefined{return action.gate?workflow?.gates[action.gate]:undefined;}
  function actionVerb(action:WorkflowAction,prepared?:typeof preparedGateReview):string{const gate=gateForAction(action);return gate?(prepared?.gate.fingerprint===gate.fingerprint?"Continue":"Review"):action.kind==="handoff"?"Hand off":action.kind==="implementation-return"?"Record return":action.kind==="verification-route"?"Review":action.id==="complete-feature"?"Complete":action.kind==="author"?"Guide":"Open";}
  ...
  function beginHandoff(action:WorkflowAction){const taskId=action.subject?.entityId;if(taskId)handoffReview={action,taskId,contractId:action.target.entityId};}
  ...
  function beginReturn(action:WorkflowAction){const taskId=action.subject?.entityId;if(!taskId)return;returnClaim="";returnKnownGaps="";returnChangedFiles="";returnAssessments='{"requirements":[],"acceptanceCriteria":[],"lockedDecisions":[]}';returnChecks="[]";returnExternalVerifications="[]";returnReview={action,taskId};}
  function claimJson(value:string,label:string,kind:"array"|"object"):unknown{let parsed:unknown;try{parsed=JSON.parse(value);}catch{throw new Error(`${label} must be valid JSON.`);}if(kind==="array"&&!Array.isArray(parsed))throw new Error(`${label} must be a JSON array.`);if(kind==="object"&&(!parsed||typeof parsed!=="object"||Array.isArray(parsed)))throw new Error(`${label} must be a JSON object.`);return parsed;}
  async function confirmReturn(){if(!returnReview||!workflowRun||recordingReturn||!returnClaim.trim())return;recordingReturn=true;error="";try{const claims=claimJson(returnAssessments,"Obligation assessments","object");const checks=claimJson(returnChecks,"Checks/tests","array");const externalVerifications=claimJson(returnExternalVerifications,"External verification","array");await api("/api/workflow/action",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({runId:workflowRun.id,action:"return",taskId:returnReview.taskId,claim:{summary:returnClaim.trim(),claims,knownGaps:returnKnownGaps.split("\n").map(value=>value.trim()).filter(Boolean),changedFiles:returnChangedFiles.split("\n").map(value=>value.trim()).filter(Boolean),checks,externalVerifications},idempotencyKey:crypto.randomUUID()})});const taskId=returnReview.taskId;returnReview=undefined;returnClaim="";returnKnownGaps="";returnChangedFiles="";await refreshWorkflow();notice=`Implementation return recorded for ${taskId}; verification must now evaluate the completion claim.`;}catch(cause){error=(cause as Error).message;}finally{recordingReturn=false;}}
  function beginVerificationReview(action:WorkflowAction){const taskId=action.subject?.entityId;if(!taskId)return;verificationReason="";verificationReview={action,taskId};}
  async function evaluateReturn(outcome:"retry-implementation"|"reconcile"|"satisfactory"){if(!verificationReview||!workflowRun||recordingReturn)return;recordingReturn=true;error="";try{const note=verificationReason.trim();await api("/api/workflow/action",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({runId:workflowRun.id,action:"evaluate-return",taskId:verificationReview.taskId,outcome,reason:outcome==="reconcile"&&note?note:undefined,blockingFindings:outcome==="retry-implementation"&&note?[note]:undefined,idempotencyKey:crypto.randomUUID()})});const taskId=verificationReview.taskId;verificationReview=undefined;verificationReason="";await refreshWorkflow();notice=outcome==="retry-implementation"?`${taskId} returned to implementation with bounded retry context.`:outcome==="reconcile"?`${taskId} routed to governing-context reconciliation.`:`${taskId} verification recorded as satisfactory.`;}catch(cause){error=(cause as Error).message;}finally{recordingReturn=false;}}
  ...
  async function performWorkflowAction(action:WorkflowAction){const gate=gateForAction(action);if(gate){await prepareGateReview(gate,action);return;}if(action.kind==="handoff"){beginHandoff(action);return;}if(action.kind==="implementation-return"){beginReturn(action);return;}if(action.kind==="verification-route"){beginVerificationReview(action);return;}if(action.id==="reopen-feature"){lifecycleValue="active";await updateLifecycle();return;}if(action.id==="cancel-stale-run"&&workflowRun){await api("/api/workflow/action",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({runId:workflowRun.id,action:"cancel",reason:"Feature remains authoritatively complete"})});await refreshWorkflow();notice=`${workflow?.featureId??"Feature"} remains complete; the stale workflow run was closed.`;return;}if(action.id==="complete-feature"&&workflowRun){if(detail?.entity.lifecycle!=="complete"){lifecycleValue="complete";await updateLifecycle();notice=`${workflow?.featureId??"Feature"} workflow completed.`;return;}await api("/api/workflow/action",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({runId:workflowRun.id,action:"complete"})});await refreshWorkflow();notice=`${workflow?.featureId??"Feature"} workflow completed.`;return;}if(action.kind==="author"){authorGuidance=action;return;}await openWorkflowTarget(action);}
...
{#if returnReview}
  <div class="modal-backdrop">
    <div class="gate-review" role="dialog" aria-modal="true" aria-labelledby="return-review-title">
      ...
      <p>Record the implementer's self-assessment. This resolves the implementation wait and starts verification; the claim does not complete or verify the task.</p>
      ...
      <label for="implementation-return-gaps"><strong>Known gaps</strong> <small>(one per line)</small></label>
      <textarea id="implementation-return-gaps" bind:value={returnKnownGaps} rows="3" placeholder="Any known incomplete behavior"></textarea>
      <label for="implementation-return-files"><strong>Changed files</strong> <small>(one per line)</small></label>
      <textarea id="implementation-return-files" bind:value={returnChangedFiles} rows="3" placeholder="src/..."></textarea>
      <label for="implementation-return-assessments"><strong>Obligation assessments</strong> <small>(JSON object)</small></label>
      <textarea id="implementation-return-assessments" bind:value={returnAssessments} rows="4"></textarea>
      <label for="implementation-return-checks"><strong>Checks and tests</strong> <small>(JSON array)</small></label>
      <textarea id="implementation-return-checks" bind:value={returnChecks} rows="3" placeholder="JSON objects with name, outcome, result, and optional command"></textarea>
      <label for="implementation-return-external"><strong>External verification requirements</strong> <small>(JSON array)</small></label>
      <textarea id="implementation-return-external" bind:value={returnExternalVerifications} rows="3" placeholder="JSON objects with description and optional verificationId"></textarea>
...
{#if verificationReview}
  <div class="modal-backdrop">
    <div class="gate-review" role="dialog" aria-modal="true" aria-labelledby="verification-return-title">
      <div class="gate-review-head"><div><p class="eyebrow">Verification coordination</p><h2 id="verification-return-title">Evaluate {verificationReview.taskId} return</h2></div><Button variant="ghost" on:click={() => verificationReview=undefined}>Close</Button></div>
      <p>Compare the implementation claim with current required verification. Retry only for a demonstrated implementation defect; missing Evidence remains pending verification. A conflict in governing truth enters reconciliation.</p>
      {#if pendingImplementationReturn?.claim.summary}<div class="review-summary"><strong>Implementer claim</strong><span>{pendingImplementationReturn.claim.summary}</span></div>{/if}
      <div class="review-summary"><strong>Unsatisfied verification</strong><span>{pendingReturnVerifications.map(value=>`${value.id} (${value.status})`).join(", ")||"None"}</span></div>
      <label for="verification-route-reason"><strong>Defect finding or governing conflict</strong></label>
      <textarea id="verification-route-reason" bind:value={verificationReason} rows="3" placeholder="Describe the observed implementation defect for retry, or the governing-context conflict for reconciliation."></textarea>
      <div class="gate-review-actions"><Button on:click={() => verificationReview=undefined}>Not yet</Button><Button disabled={recordingReturn||!pendingReturnVerifications.length||!verificationReason.trim()} on:click={() => evaluateReturn("retry-implementation")}>Retry implementation</Button><Button disabled={recordingReturn||!verificationReason.trim()} on:click={() => evaluateReturn("reconcile")}>Reconcile context</Button><Button variant="primary" disabled={recordingReturn||pendingReturnVerifications.length>0} on:click={() => evaluateReturn("satisfactory")}>Verification satisfactory</Button></div>
    </div>
  </div>
{/if}
