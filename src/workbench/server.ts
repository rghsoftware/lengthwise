    ...
    async fetch(request) {
      ...
      try {
        ...
        if (url.pathname === "/api/workflow/action" && request.method === "POST") {
          ...
          else if(body.action==="return"&&typeof body.taskId==="string"&&(typeof body.claim==="string"||(typeof body.claim==="object"&&body.claim!==null))&&typeof body.idempotencyKey==="string")result=await workflow.returnImplementation(body.runId,body.taskId,body.claim as import("../workflow/coordinator.ts").ImplementationCompletionClaim|string,body.idempotencyKey);
          else if(body.action==="evaluate-return"&&typeof body.taskId==="string"&&["retry-implementation","reconcile","satisfactory"].includes(String(body.outcome))&&typeof body.idempotencyKey==="string")result=await workflow.evaluateImplementationReturn(body.runId,{taskId:body.taskId,outcome:body.outcome as "retry-implementation"|"reconcile"|"satisfactory",failedVerifications:Array.isArray(body.failedVerifications)?body.failedVerifications.filter((value):value is string=>typeof value==="string"):undefined,blockingFindings:Array.isArray(body.blockingFindings)?body.blockingFindings.filter((value):value is string=>typeof value==="string"):undefined,knownGaps:Array.isArray(body.knownGaps)?body.knownGaps.filter((value):value is string=>typeof value==="string"):undefined,reason:typeof body.reason==="string"?body.reason:undefined,idempotencyKey:body.idempotencyKey});
