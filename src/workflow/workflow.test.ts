...
test("state schema migration is transactional, typed, and adds idempotent attempts",async()=>{
  ...
  const store=new WorkflowStateStore(path); expect(store.schemaVersion()).toBe(6); expect(store.get("R")?.activity).toBe("plan");
  ...
  store.finishAttempt(first.id,"interrupted"); expect(store.retryAttempt(first.id,"fp").state).toBe("running");store.wait("R","implementation","TASK");expect(()=>store.wait("R","implementation","TASK")).toThrow();
  expect(()=>store.update("R","unknown" as any,"running")).toThrow("Unsupported workflow activity"); store.close();
});

test("concurrent first-open migration serializes across synchronized processes",async()=>{
  const root=await createFixtureRepo({".keep":""});roots.push(root);const path=`${root}/state.db`;const gate=`${root}/open-migration`;const moduleUrl=new URL("./state-store.ts",import.meta.url).href;
  const processes=Array.from({length:12},(_,index)=>{const ready=`${root}/ready-${index}`;const source=`import { WorkflowStateStore } from ${JSON.stringify(moduleUrl)}; await Bun.write(${JSON.stringify(ready)}, "ready"); while (!(await Bun.file(${JSON.stringify(gate)}).exists())) await Bun.sleep(1); const store = new WorkflowStateStore(${JSON.stringify(path)}); if (store.schemaVersion() !== 6) process.exit(2); store.close();`;return Bun.spawn([process.execPath,"-e",source],{stdout:"pipe",stderr:"pipe"});});
  while (!(await Promise.all(Array.from({length:12},(_,index)=>Bun.file(`${root}/ready-${index}`).exists()))).every(Boolean)) await Bun.sleep(1);await Bun.write(gate,"open");
  const exits=await Promise.all(processes.map(process=>process.exited));
  expect(exits).toEqual(Array(12).fill(0));
  const store=new WorkflowStateStore(path);expect(store.schemaVersion()).toBe(6);store.close();
});

test("workflow events retain insertion order when timestamps and IDs disagree",async()=>{
  const root=await createFixtureRepo({".keep":""});roots.push(root);const path=`${root}/state.db`;let store=new WorkflowStateStore(path);const run=store.start("F-ORDER");store.event(run.id,"verification-reconciliation-required",{reason:"active"},undefined,"z-route");store.event(run.id,"reconciliation-complete",{},undefined,"a-complete");store.close();const db=new Database(path);db.query("UPDATE workflow_events SET created_at=? WHERE run_id=?").run("2026-09-03T00:00:00.000Z",run.id);db.close();store=new WorkflowStateStore(path);expect(store.events(run.id).map(event=>event.kind)).toEqual(["verification-reconciliation-required","reconciliation-complete"]);store.close();
});

test("workflow attempts retain insertion order when timestamps and IDs disagree",async()=>{
  const root=await createFixtureRepo({".keep":""});roots.push(root);const path=`${root}/state.db`;let store=new WorkflowStateStore(path);const run=store.start("F-ATTEMPT-ORDER");const first=store.beginAttempt(run.id,"evaluate-return:first","same");store.finishAttempt(first.id,"succeeded",{taskId:"TASK"});const second=store.beginAttempt(run.id,"evaluate-return:second","same");store.finishAttempt(second.id,"succeeded",{taskId:"TASK"});store.close();const db=new Database(path);db.query("UPDATE workflow_attempts SET created_at=?,updated_at=?,id=? WHERE id=?").run("2026-09-03T00:00:00.000Z","2026-09-03T00:00:00.000Z","z-first",first.id);db.query("UPDATE workflow_attempts SET created_at=?,updated_at=?,id=? WHERE id=?").run("2026-09-03T00:00:00.000Z","2026-09-03T00:00:00.000Z","a-second",second.id);db.close();store=new WorkflowStateStore(path);expect(store.attempts(run.id).map(attempt=>attempt.id)).toEqual(["z-first","a-second"]);store.close();
});
...
test("capture from an idea creates a draft Feature artifact and associated run",async()=>{
  ...
  expect(captured.run.featureId).toBe("F-IDEA");expect(captured.run.activity).toBe("specify");expect(await Bun.file(`${root}/engineering/features/idea.yaml`).text()).toContain("lifecycle: draft");expect(captured.assessment.actions.some(a=>a.target.entityId==="F-IDEA")).toBe(true);expect(Object.getOwnPropertyNames(coordinator)).not.toContain("state");expect(Reflect.get(coordinator,"state")).toBeUndefined();coordinator.close();
});
...
`;
  const root=await createFixtureRepo({".lengthwise/project.yaml":WORKFLOW_CONFIG,"engineering/model.yaml":model});roots.push(root);const fixtureState=new WorkflowStateStore(`${root}/.lengthwise/state.db`);fixtureState.start("F-DONE","specify");fixtureState.close();const coordinator=await WorkflowCoordinator.open(root);
  await expect(coordinator.start("F-DONE")).rejects.toThrow("reopen it to active");const assessment=await coordinator.assess("F-DONE");
...
const STRICT_CONFIG=WORKFLOW_CONFIG.replace("policy: { rigor: light }","policy: { rigor: strict }");
...
test("handoff, interruption, resume, retry, return verification, completion, and terminal projection converge",async()=>{
  ...
  await coordinator.handoff(run.id,"TASK-FULL","handoff-key");assessed=await coordinator.assess("F-FULL");expect(assessed.actions.some(action=>action.id==="handoff:TASK-FULL")).toBe(false);expect(assessed.actions.find(action=>action.id==="return:TASK-FULL")?.kind).toBe("implementation-return");const fixtureState=new WorkflowStateStore(`${root}/.lengthwise/state.db`);const authored=fixtureState.beginAttempt(run.id,"author:progress","progress-key",assessed.fingerprint);fixtureState.close();expect(coordinator.interrupt(run.id,"pause").state).toBe("interrupted");const resumed=await coordinator.resume(run.id);expect(resumed.classifications[0]?.classification).toBe("no-write-observed");expect((await coordinator.retry(run.id,authored.id)).state).toBe("running");const completedState=new WorkflowStateStore(`${root}/.lengthwise/state.db`);completedState.finishAttempt(authored.id,"succeeded");completedState.close();
  const returned=await coordinator.returnImplementation(run.id,"TASK-FULL","implementation finished","return-key");expect(returned.result).toEqual(expect.objectContaining({remaining:[],claim:expect.objectContaining({taskId:"TASK-FULL",acceptedBuildContract:expect.objectContaining({id:"BC-TASK-FULL",fingerprint:expect.any(String)}),implementationAttemptId:expect.any(String),claims:{requirements:[],acceptanceCriteria:[],lockedDecisions:[]},knownGaps:[],changedFiles:[],checks:[],externalVerifications:[]})}));
  ...
  const terminal=await coordinator.assess("F-FULL");expect(terminal.gates.specification.approved).toBe(true);expect(terminal.gates["build-contract"].approved).toBe(true);expect(terminal.completionEligible).toBe(true);expect(terminal.actions).toEqual([]);await Bun.write(`${root}/engineering/model.yaml`,FULL_MODEL.replace("lifecycle: planned","lifecycle: done"));expect((await coordinator.assess("F-FULL")).actions).toEqual([]);coordinator.close();
});

test("strict verification approval can complete through reconciliation after the authoritative lifecycle save",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STRICT_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);let built=await buildProjectGraph(root);if(!built.ok)throw new Error("fixture failed");await Bun.write(`${root}/engineering/contracts.yaml`,renderContractArtifact(built.graph,["TASK-FULL"]));const coordinator=await WorkflowCoordinator.open(root);const run=await coordinator.start("F-FULL");let assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"specification",assessment.gates.specification.fingerprint);assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"build-contract",assessment.gates["build-contract"].fingerprint);await coordinator.handoff(run.id,"TASK-FULL","handoff");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Implemented"},"return");
  await Bun.write(`${root}/engineering/model.yaml`,FULL_MODEL.replace("lifecycle: planned","lifecycle: done"));built=await buildProjectGraph(root);if(!built.ok)throw new Error("updated fixture failed");const evidenceFingerprint=verificationContextFingerprint(built.graph,"VER-FULL");await Bun.write(`${root}/engineering/evidence.yaml`,`lengthwise: 1\nentities:\n  - { id: E-STRICT, type: evidence, title: Result, lifecycle: recorded, outcome: passed, result: Passed, applicability: current, contextFingerprint: ${evidenceFingerprint}, relationships: [{ type: supports, to: VER-FULL }] }\n`);await coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"satisfactory",idempotencyKey:"verified"});assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"verification",assessment.gates.verification.fingerprint);expect(coordinator.getRun(run.id)?.activity).toBe("reconcile");expect((await coordinator.assess("F-FULL")).completionEligible).toBe(true);
  await Bun.write(`${root}/engineering/model.yaml`,FULL_MODEL.replace("lifecycle: draft","lifecycle: complete").replace("lifecycle: planned","lifecycle: done"));expect((await coordinator.complete(run.id)).state).toBe("complete");coordinator.close();
});

test("invalid reconciliation route is rejected before workflow state changes",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":WORKFLOW_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const coordinator=await WorkflowCoordinator.open(root);const run=await coordinator.start("F-FULL");await expect(coordinator.reconcile(run.id,"bogus" as never,"invalid route")).rejects.toThrow("Unsupported reconciliation route");expect(coordinator.getRun(run.id)).toMatchObject({activity:"specify",state:"running"});expect((await coordinator.inspectFeature("F-FULL")).events).toEqual([]);coordinator.close();
});

test("cancellation preserves terminal history and permits a later run",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":WORKFLOW_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const coordinator=await WorkflowCoordinator.open(root);const first=await coordinator.start("F-FULL");expect(coordinator.cancel(first.id,"not now").state).toBe("cancelled");const second=await coordinator.start("F-FULL");expect(second.id).not.toBe(first.id);expect((await coordinator.inspectFeature("F-FULL")).history).toHaveLength(2);coordinator.close();
});

test("completed tasks are not offered for implementation handoff",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":FULL_MODEL.replace("lifecycle: planned","lifecycle: done")});roots.push(root);const built=await buildProjectGraph(root);if(!built.ok)throw new Error("fixture failed");await Bun.write(`${root}/engineering/contracts.yaml`,renderContractArtifact(built.graph,["TASK-FULL"]));const coordinator=await WorkflowCoordinator.open(root);const run=await coordinator.start("F-FULL");let assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"specification",assessment.gates.specification.fingerprint);assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"build-contract",assessment.gates["build-contract"].fingerprint);assessment=await coordinator.assess("F-FULL");expect(assessment.tasks[0]?.lifecycle).toBe("done");expect(assessment.actions.some(action=>action.id==="handoff:TASK-FULL")).toBe(false);expect(coordinator.getRun(run.id)?.activity).toBe("verify");coordinator.close();
});

test("strict verification remains reachable when implementation was already complete",async()=>{
  const model=FULL_MODEL.replace("lifecycle: planned","lifecycle: done");const root=await createFixtureRepo({".lengthwise/project.yaml":STRICT_CONFIG,"engineering/model.yaml":model});roots.push(root);let built=await buildProjectGraph(root);if(!built.ok)throw new Error("fixture failed");await Bun.write(`${root}/engineering/contracts.yaml`,renderContractArtifact(built.graph,["TASK-FULL"]));built=await buildProjectGraph(root);if(!built.ok)throw new Error("contract fixture failed");const evidenceFingerprint=verificationContextFingerprint(built.graph,"VER-FULL");await Bun.write(`${root}/engineering/evidence.yaml`,`lengthwise: 1\nentities:\n  - { id: E-PRECOMPLETED, type: evidence, title: Result, lifecycle: recorded, outcome: passed, result: Passed, applicability: current, contextFingerprint: ${evidenceFingerprint}, relationships: [{ type: supports, to: VER-FULL }] }\n`);const coordinator=await WorkflowCoordinator.open(root);const run=await coordinator.start("F-FULL");let assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"specification",assessment.gates.specification.fingerprint);assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"build-contract",assessment.gates["build-contract"].fingerprint);expect(coordinator.getRun(run.id)?.activity).toBe("verify");assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"verification",assessment.gates.verification.fingerprint);expect(coordinator.getRun(run.id)?.activity).toBe("reconcile");coordinator.close();
});
...
test("returning one of multiple handed-off tasks preserves the other implementation wait",async()=>{
  ...
  const returned=await coordinator.returnImplementation(run.id,"TASK-FULL","first done","r1");expect(returned.result).toEqual(expect.objectContaining({remaining:["TASK-SECOND"]}));expect(coordinator.getRun(run.id)).toMatchObject({activity:"implement",state:"waiting-implementation"});const fixtureState=new WorkflowStateStore(`${root}/.lengthwise/state.db`);expect(fixtureState.waiting(run.id,"implementation")).toEqual([{targetId:"TASK-SECOND"}]);fixtureState.close();coordinator.close();
});
...
test("implementation handoff is atomic and idempotent across coordinator instances",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const {coordinator,run}=await readyRun(root);const other=await WorkflowCoordinator.open(root);const [first,replay]=await Promise.all([coordinator.handoff(run.id,"TASK-FULL","same-key"),other.handoff(run.id,"TASK-FULL","same-key")]);expect(replay.id).toBe(first.id);expect(replay.result).toEqual(first.result);await expect(other.handoff(run.id,"TASK-FULL","different-key")).rejects.toThrow("already has an outstanding implementation handoff");const view=await coordinator.inspectFeature("F-FULL");expect(view.events.filter(event=>event.kind==="implementation-handed-off")).toHaveLength(1);expect(view.attempts.filter(attempt=>attempt.actionId==="handoff:TASK-FULL")).toHaveLength(1);expect(view.assessment.actions.filter(action=>action.id==="return:TASK-FULL")).toHaveLength(1);other.close();coordinator.close();
});

test("missing Evidence alone remains pending verification instead of causing implementation retry",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const {coordinator,run}=await readyRun(root);await coordinator.handoff(run.id,"TASK-FULL","handoff");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Implementation returned"},"return");await expect(coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"retry-implementation",failedVerifications:["VER-FULL"],idempotencyKey:"route"})).rejects.toThrow("missing Evidence alone remains pending verification");const assessment=await coordinator.assess("F-FULL");expect(coordinator.getRun(run.id)?.activity).toBe("verify");expect(assessment.implementation.pendingReturns).toHaveLength(1);expect(assessment.implementation.retryContexts).toEqual([]);coordinator.close();
});

test("outstanding handoffs and unevaluated returns block verification and completion",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STRICT_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const {coordinator,run}=await readyRun(root);await coordinator.handoff(run.id,"TASK-FULL","handoff");await Bun.write(`${root}/engineering/model.yaml`,FULL_MODEL.replace("lifecycle: planned","lifecycle: done"));let built=await buildProjectGraph(root);if(!built.ok)throw new Error("updated fixture failed");const evidenceFingerprint=verificationContextFingerprint(built.graph,"VER-FULL");await Bun.write(`${root}/engineering/evidence.yaml`,`lengthwise: 1\nentities:\n  - { id: E-COORDINATION, type: evidence, title: Result, lifecycle: recorded, outcome: passed, result: Passed, applicability: current, contextFingerprint: ${evidenceFingerprint}, relationships: [{ type: supports, to: VER-FULL }] }\n`);let assessment=await coordinator.assess("F-FULL");expect(assessment.gates.verification.eligible).toBe(false);expect(assessment.completionEligible).toBe(false);expect(assessment.gates.verification.blockers).toContainEqual(expect.objectContaining({code:"implementation-return-pending",entityId:"TASK-FULL"}));await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Returned but not independently reviewed"},"return");assessment=await coordinator.assess("F-FULL");expect(assessment.gates.verification.eligible).toBe(false);expect(assessment.completionEligible).toBe(false);expect(assessment.gates.verification.blockers).toContainEqual(expect.objectContaining({code:"implementation-return-pending-verification",entityId:"TASK-FULL"}));await expect(coordinator.approve(run.id,"verification",assessment.gates.verification.fingerprint)).rejects.toThrow("ineligible");coordinator.close();
});

test("retry defect claims are scoped to the accepted task contract",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const {coordinator,run}=await readyRun(root);await coordinator.handoff(run.id,"TASK-FULL","handoff");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Claim references unrelated work",claims:{requirements:[{id:"REQ-UNRELATED",state:"not-addressed"}]}},"return");await expect(coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"retry-implementation",failedVerifications:["VER-FULL"],idempotencyKey:"route"})).rejects.toThrow("missing Evidence alone remains pending verification");expect((await coordinator.assess("F-FULL")).implementation.pendingReturns).toHaveLength(1);coordinator.close();
});

test("current failing Evidence establishes a defect even when complementary Evidence is missing",async()=>{
  const model=FULL_MODEL.replace("method: automated, required: true","method: automated, required: true, evidenceRequirements: [test, review]");const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":model});roots.push(root);const {coordinator,run}=await readyRun(root);await coordinator.handoff(run.id,"TASK-FULL","handoff");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Implementation returned"},"return");const built=await buildProjectGraph(root);if(!built.ok)throw new Error("fixture failed");const evidenceFingerprint=verificationContextFingerprint(built.graph,"VER-FULL");await Bun.write(`${root}/engineering/evidence.yaml`,`lengthwise: 1\nentities:\n  - { id: E-FAILED, type: evidence, title: Failed test, lifecycle: recorded, outcome: failed, result: Assertion failed, applicability: current, contextFingerprint: ${evidenceFingerprint}, kind: test, relationships: [{ type: supports, to: VER-FULL }] }\n`);const before=await coordinator.assess("F-FULL");expect(before.verifications[0]).toMatchObject({status:"missing-complement",failingEvidenceIds:["E-FAILED"]});await expect(coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"retry-implementation",failedVerifications:["VER-FULL"],idempotencyKey:"route"})).resolves.toMatchObject({state:"succeeded"});coordinator.close();
});
...
test("false implementation completion claim returns visible unmet obligations to implementation",async()=>{
  ...
  await coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"retry-implementation",failedVerifications:["VER-FULL"],blockingFindings:["Expected behavior was not observed"],knownGaps:["AC behavior missing"],idempotencyKey:"route-1"});assessment=await coordinator.assess("F-FULL");const retry=assessment.implementation.retryContexts[0];expect(coordinator.getRun(run.id)?.activity).toBe("implement");expect(retry).toEqual(expect.objectContaining({taskId:"TASK-FULL",failedVerifications:["VER-FULL"],affectedAcceptanceCriteria:["AC-FULL"],affectedRequirements:["REQ-FULL"],contractId:"BC-TASK-FULL",contractCurrent:true,nextEligibleAction:"implementation-handoff"}));expect(assessment.actions.find(action=>action.id==="handoff:TASK-FULL")?.requiredInputs.join(" ")).toContain("AC-FULL");coordinator.close();
});

test("a successful implementation retry reuses the accepted contract and scopes a repeated caller key to its attempt",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const {coordinator,run}=await readyRun(root);await coordinator.handoff(run.id,"TASK-FULL","handoff-1");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Attempt one complete"},"return-1");await coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"retry-implementation",failedVerifications:["VER-FULL"],blockingFindings:["Required behavior remains absent"],idempotencyKey:"route-1"});const second=await coordinator.handoff(run.id,"TASK-FULL","handoff-2");expect(second.result).toEqual(expect.objectContaining({attemptNumber:2,contractId:"BC-TASK-FULL"}));
  await Bun.write(`${root}/engineering/model.yaml`,FULL_MODEL.replace("lifecycle: planned","lifecycle: done"));const built=await buildProjectGraph(root);if(!built.ok)throw new Error("updated fixture failed");const evidenceFingerprint=verificationContextFingerprint(built.graph,"VER-FULL");await Bun.write(`${root}/engineering/evidence.yaml`,`lengthwise: 1\nentities:\n  - { id: E-FULL, type: evidence, title: Result, lifecycle: recorded, outcome: passed, result: Passed, applicability: current, contextFingerprint: ${evidenceFingerprint}, relationships: [{ type: supports, to: VER-FULL }] }\n`);await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Missing behavior implemented",knownGaps:[],changedFiles:["src/feature.ts"]},"return-2");const verified=await coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"satisfactory",idempotencyKey:"route-1"});const replay=await coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"satisfactory",idempotencyKey:"route-1"});expect(replay.id).toBe(verified.id);expect(replay.result).toEqual(expect.objectContaining({verified:["VER-FULL"]}));const assessment=await coordinator.assess("F-FULL");expect(assessment.implementation.attempts).toHaveLength(2);expect(assessment.implementation.pendingReturns).toEqual([]);expect(assessment.gates["build-contract"].approved).toBe(true);expect(assessment.completionEligible).toBe(true);expect(assessment.reconciliation.required).toBe(false);coordinator.close();
});

test("verification routes changed governing truth to reconciliation instead of implementation retry",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const {coordinator,run}=await readyRun(root);await coordinator.handoff(run.id,"TASK-FULL","handoff-1");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Contract cannot satisfy the platform constraint",claims:{acceptanceCriteria:[{id:"AC-FULL",state:"not-addressed"}]}},"return-1");await coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"reconcile",failedVerifications:["VER-FULL"],reason:"The accepted criterion conflicts with the discovered platform constraint",idempotencyKey:"route-1"});const assessment=await coordinator.assess("F-FULL");expect(coordinator.getRun(run.id)?.activity).toBe("reconcile");expect(assessment.reconciliation.reasons[0]).toEqual(expect.objectContaining({code:"governing-context-conflict",entityId:"TASK-FULL"}));expect(assessment.implementation.retryContexts).toEqual([]);expect(assessment.actions.some(action=>action.id==="handoff:TASK-FULL"&&action.eligible)).toBe(false);coordinator.close();
});

test("reconciliation completion cannot discard another task's unevaluated return",async()=>{
  const model=FULL_MODEL.replace("  - { id: PLAN-FULL",`  - { id: TASK-SECOND, type: task, title: Implement second, lifecycle: planned, relationships: [{ type: implements, to: REQ-FULL }] }\n  - { id: PLAN-FULL`).replace("relationships: [{ type: contains, to: TASK-FULL }]","relationships: [{ type: contains, to: TASK-FULL }, { type: contains, to: TASK-SECOND }]");const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":model});roots.push(root);let built=await buildProjectGraph(root);if(!built.ok)throw new Error("fixture failed");await Bun.write(`${root}/engineering/contracts.yaml`,renderContractArtifact(built.graph,["TASK-FULL","TASK-SECOND"]));const coordinator=await WorkflowCoordinator.open(root);const run=await coordinator.start("F-FULL");let assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"specification",assessment.gates.specification.fingerprint);assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"build-contract",assessment.gates["build-contract"].fingerprint);await coordinator.handoff(run.id,"TASK-FULL","handoff-one");await coordinator.handoff(run.id,"TASK-SECOND","handoff-two");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"First conflict"},"return-one");await coordinator.returnImplementation(run.id,"TASK-SECOND",{summary:"Second awaits review"},"return-two");await coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"reconcile",reason:"First task conflicts with governing truth",idempotencyKey:"route-one"});await expect(coordinator.reconcile(run.id,"complete","Conflict resolved")).rejects.toThrow("TASK-SECOND has pending implementation coordination");assessment=await coordinator.assess("F-FULL");expect(assessment.implementation.pendingReturns.map(item=>item.taskId)).toEqual(["TASK-SECOND"]);expect(assessment.actions.some(action=>action.id==="review-return:TASK-SECOND")).toBe(true);coordinator.close();
});

test("verification idempotency keys are scoped to the returned task",async()=>{
  const model=FULL_MODEL.replace("  - { id: PLAN-FULL",`  - { id: TASK-SECOND, type: task, title: Implement second, lifecycle: planned, relationships: [{ type: implements, to: REQ-FULL }] }\n  - { id: PLAN-FULL`).replace("relationships: [{ type: contains, to: TASK-FULL }]","relationships: [{ type: contains, to: TASK-FULL }, { type: contains, to: TASK-SECOND }]");
  const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":model});roots.push(root);let built=await buildProjectGraph(root);if(!built.ok)throw new Error("fixture failed");await Bun.write(`${root}/engineering/contracts.yaml`,renderContractArtifact(built.graph,["TASK-FULL","TASK-SECOND"]));const coordinator=await WorkflowCoordinator.open(root);const run=await coordinator.start("F-FULL");let assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"specification",assessment.gates.specification.fingerprint);assessment=await coordinator.assess("F-FULL");await coordinator.approve(run.id,"build-contract",assessment.gates["build-contract"].fingerprint);await coordinator.handoff(run.id,"TASK-FULL","handoff-one");await coordinator.handoff(run.id,"TASK-SECOND","handoff-two");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"First returned"},"return-one");await coordinator.returnImplementation(run.id,"TASK-SECOND",{summary:"Second returned"},"return-two");
  await Bun.write(`${root}/engineering/model.yaml`,model.replaceAll("lifecycle: planned","lifecycle: done"));built=await buildProjectGraph(root);if(!built.ok)throw new Error("updated fixture failed");const evidenceFingerprint=verificationContextFingerprint(built.graph,"VER-FULL");await Bun.write(`${root}/engineering/evidence.yaml`,`lengthwise: 1\nentities:\n  - { id: E-SHARED, type: evidence, title: Shared result, lifecycle: recorded, outcome: passed, result: Passed, applicability: current, contextFingerprint: ${evidenceFingerprint}, relationships: [{ type: supports, to: VER-FULL }] }\n`);
  const first=await coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"satisfactory",idempotencyKey:"shared-key"});const second=await coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-SECOND",outcome:"satisfactory",idempotencyKey:"shared-key"});expect(first.id).not.toBe(second.id);expect(second.result).toEqual(expect.objectContaining({taskId:"TASK-SECOND"}));assessment=await coordinator.assess("F-FULL");expect(assessment.implementation.pendingReturns).toEqual([]);expect((await coordinator.inspectFeature("F-FULL")).attempts.filter(attempt=>attempt.actionId.startsWith("evaluate-return:"))).toHaveLength(2);coordinator.close();
});

test("recorded Evidence resolves a claimed external Verification",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const {coordinator,run}=await readyRun(root);await coordinator.handoff(run.id,"TASK-FULL","handoff");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Implementation returned",externalVerifications:[{verificationId:"VER-FULL",description:"Confirm the workflow behavior"}]},"return");await expect(coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"satisfactory",idempotencyKey:"before-evidence"})).rejects.toThrow("external verification pending");await Bun.write(`${root}/engineering/model.yaml`,FULL_MODEL.replace("lifecycle: planned","lifecycle: done"));const built=await buildProjectGraph(root);if(!built.ok)throw new Error("updated fixture failed");const evidenceFingerprint=verificationContextFingerprint(built.graph,"VER-FULL");await Bun.write(`${root}/engineering/evidence.yaml`,`lengthwise: 1\nentities:\n  - { id: E-EXTERNAL, type: evidence, title: External result, lifecycle: recorded, outcome: passed, result: Passed, applicability: current, contextFingerprint: ${evidenceFingerprint}, relationships: [{ type: supports, to: VER-FULL }] }\n`);await expect(coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"satisfactory",idempotencyKey:"after-evidence"})).resolves.toMatchObject({state:"succeeded",result:expect.objectContaining({taskId:"TASK-FULL"})});coordinator.close();
});

test("Evidence can resolve an optional contract-scoped external Verification",async()=>{
  const model=FULL_MODEL.replace("  - { id: TASK-FULL",`  - { id: VER-OPTIONAL, type: verification, title: Optional external review, method: review, required: false, lifecycle: defined, relationships: [{ type: verifies, to: AC-FULL }] }\n  - { id: TASK-FULL`);const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":model});roots.push(root);const {coordinator,run}=await readyRun(root);await coordinator.handoff(run.id,"TASK-FULL","handoff");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Implementation returned",externalVerifications:[{verificationId:"VER-OPTIONAL",description:"Independent review"}]},"return");await Bun.write(`${root}/engineering/model.yaml`,model.replace("lifecycle: planned","lifecycle: done"));const built=await buildProjectGraph(root);if(!built.ok)throw new Error("updated fixture failed");const requiredFingerprint=verificationContextFingerprint(built.graph,"VER-FULL");const optionalFingerprint=verificationContextFingerprint(built.graph,"VER-OPTIONAL");await Bun.write(`${root}/engineering/evidence.yaml`,`lengthwise: 1\nentities:\n  - { id: E-REQUIRED, type: evidence, title: Required result, lifecycle: recorded, outcome: passed, result: Passed, applicability: current, contextFingerprint: ${requiredFingerprint}, relationships: [{ type: supports, to: VER-FULL }] }\n  - { id: E-OPTIONAL, type: evidence, title: Optional review, lifecycle: recorded, outcome: passed, result: Approved, applicability: current, contextFingerprint: ${optionalFingerprint}, relationships: [{ type: supports, to: VER-OPTIONAL }] }\n`);await expect(coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"satisfactory",idempotencyKey:"optional-external"})).resolves.toMatchObject({state:"succeeded"});coordinator.close();
});

test("a stale accepted Build Contract cannot receive satisfactory verification",async()=>{
  const root=await createFixtureRepo({".lengthwise/project.yaml":STANDARD_CONFIG,"engineering/model.yaml":FULL_MODEL});roots.push(root);const {coordinator,run}=await readyRun(root);await coordinator.handoff(run.id,"TASK-FULL","handoff");await coordinator.returnImplementation(run.id,"TASK-FULL",{summary:"Implementation returned"},"return");await Bun.write(`${root}/engineering/model.yaml`,FULL_MODEL.replace("Complete the workflow","Materially changed governing behavior").replace("lifecycle: planned","lifecycle: done"));await expect(coordinator.evaluateImplementationReturn(run.id,{taskId:"TASK-FULL",outcome:"satisfactory",idempotencyKey:"stale-route"})).rejects.toThrow("stale; route governing-context changes to reconciliation");expect((await coordinator.assess("F-FULL")).implementation.pendingReturns).toHaveLength(1);coordinator.close();
});
