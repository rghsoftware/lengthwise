...
const ACTIVITY_TRANSITIONS:Record<WorkflowActivity,readonly WorkflowActivity[]>={capture:["specify"],specify:["plan","reconcile"],plan:["specify","implement","verify","reconcile"],implement:["verify","reconcile"],verify:["implement","reconcile","complete"],reconcile:["specify","plan","implement","verify","complete"],complete:[]};
...
export class WorkflowStateStore {
  ...
  #immediate<T>(operation: () => T): T { return this.#db.inTransaction ? operation() : this.#db.transaction(operation).immediate(); }
  private migrate(): void {
    this.#db.exec("PRAGMA busy_timeout=30000; PRAGMA foreign_keys=ON;");
    let version = 0;
    try {
      this.#db.transaction(() => {
        this.#db.exec("CREATE TABLE IF NOT EXISTS schema_version(version INTEGER NOT NULL); INSERT INTO schema_version SELECT 0 WHERE NOT EXISTS (SELECT 1 FROM schema_version);");
        version = Number((this.#db.query("SELECT version FROM schema_version LIMIT 1").get() as { version: number } | null)?.version ?? 0);
        ...
        if(next===3){
          ...
          this.#db.query(`UPDATE workflow_waits SET state='resolved',resolved_at=?
            WHERE kind='implementation' AND state='waiting' AND target_id IS NOT NULL
              AND id NOT IN (SELECT MIN(id) FROM workflow_waits WHERE kind='implementation' AND state='waiting' AND target_id IS NOT NULL GROUP BY run_id,target_id)`).run(migratedAt);
          this.#db.exec("CREATE UNIQUE INDEX one_waiting_implementation_per_target ON workflow_waits(run_id,target_id) WHERE kind='implementation' AND state='waiting' AND target_id IS NOT NULL;");
          next=4;
        }
        if(next===4){
          this.#db.exec(`ALTER TABLE workflow_events ADD COLUMN sequence INTEGER;
            UPDATE workflow_events SET sequence=rowid WHERE sequence IS NULL;
            CREATE UNIQUE INDEX workflow_event_sequence ON workflow_events(run_id,sequence);`);
          next=5;
        }
        if(next===5){
          this.#db.exec(`ALTER TABLE workflow_attempts ADD COLUMN sequence INTEGER;
            UPDATE workflow_attempts SET sequence=rowid WHERE sequence IS NULL;
            CREATE UNIQUE INDEX workflow_attempt_sequence ON workflow_attempts(run_id,sequence);`);
          next=6;
        }
  ...
  immediateTransaction<T>(operation: () => T): T { return this.#immediate(operation); }
  ...
  event(runId: string, kind: string, payload: unknown, fingerprint?: string, id: string = crypto.randomUUID()): WorkflowEvent {
    return this.#immediate(() => { if (!this.get(runId)) throw new Error(`Unknown workflow run ${runId}`); const now = new Date().toISOString(); this.#db.query(`INSERT OR IGNORE INTO workflow_events(id,run_id,kind,context_fingerprint,payload,created_at,sequence)
      SELECT ?,?,?,?,?,?,COALESCE(MAX(sequence),0)+1 FROM workflow_events WHERE run_id=?`).run(id, runId, kind, fingerprint ?? null, JSON.stringify(payload), now, runId); return this.events(runId).find(event => event.id === id)!; });
  }
  events(runId: string): WorkflowEvent[] { return (this.#db.query("SELECT id,run_id runId,kind,context_fingerprint contextFingerprint,payload,created_at createdAt FROM workflow_events WHERE run_id=? ORDER BY sequence,id").all(runId) as Array<Omit<WorkflowEvent,"payload"> & {payload:string}>).map(e => ({...e, payload: JSON.parse(e.payload)})); }
  ...
  beginAttempt(runId: string, actionId: string, idempotencyKey: string, repositoryFingerprint?: string): WorkflowAttempt {
    return this.#immediate(() => { if(!this.get(runId))throw new Error(`Unknown workflow run ${runId}`);const existing = this.attemptByKey(runId,actionId,idempotencyKey); if (existing) return existing; const now = new Date().toISOString(); const id = crypto.randomUUID();
      this.#db.query(`INSERT INTO workflow_attempts(id,run_id,action_id,idempotency_key,state,repository_fingerprint,result,created_at,updated_at,sequence)
        SELECT ?,?,?,?,?,?,?,?,?,COALESCE(MAX(sequence),0)+1 FROM workflow_attempts WHERE run_id=?`).run(id, runId, actionId, idempotencyKey, "running", repositoryFingerprint ?? null, null, now, now, runId); return this.attemptByKey(runId,actionId,idempotencyKey)!; });
  }
  ...
  attempts(runId: string): WorkflowAttempt[] { return (this.#db.query("SELECT id,run_id runId,action_id actionId,idempotency_key idempotencyKey,state,repository_fingerprint repositoryFingerprint,result,created_at createdAt,updated_at updatedAt FROM workflow_attempts WHERE run_id=? ORDER BY sequence,id").all(runId) as Array<Omit<WorkflowAttempt,"result"> & {result:string|null}>).map(a=>({...a,result:a.result?JSON.parse(a.result):undefined})); }
